...
### Browser Testing
The game was tested on Iphone as well by a friend who reported no bugs and all features working. <br>
...
- During testing, the application was working perfectly on all browsers including Firefox. However, during more in-depth testing a bug was discovered only on Firefox on android. The bug was displaying two words at the same time and incrementing the score by 2. Dev tools and a debugger were used to pinpoint the issue. It seems that the function to generate words was being triggered twice. The bug was fixed by replacing the input event with keyup event on the input field event listener. This fixed the issue and the game and all the features were then tested on all browsers to make sure that no other functionality was affected by this fix.
